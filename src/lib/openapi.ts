...
const domainSchemas = {
  ...
  RoundDetail: object({ ...roundFields, creatorId: id, isCreator: { type: 'boolean' }, members: array(ref('RoundMember')), expenses: array(ref('Expense')), expensesNextCursor: { type: 'string', nullable: true } }, [...Object.keys(roundFields), 'creatorId', 'isCreator', 'members', 'expenses', 'expensesNextCursor']),
...
const domainPaths = {
  ...
  '/api/rounds/{roundId}': {
    get: operation('정산', '회차·지출·참여 내역 조회', { response: ref('RoundDetail'), parameters: pageParameters, description: '같은 DB 스냅샷의 원본과 버전을 반환합니다. 제외·모임 이탈 후에도 참여 이력으로 조회할 수 있고 타인 계좌는 반환하지 않습니다.' }),
