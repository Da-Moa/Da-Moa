...
const domainSchemas = {
  ...
  RoundDetail: object({ ...roundFields, creatorId: id, isCreator: { type: 'boolean' }, members: array(ref('RoundMember')), expenses: array(ref('Expense')), expensesNextCursor: { type: 'string', nullable: true }, transfers: array(ref('SettlementTransfer')) }, [...Object.keys(roundFields), 'creatorId', 'isCreator', 'members', 'expenses', 'expensesNextCursor', 'transfers']),
...
const domainPaths = {
  ...
  '/api/rounds/{roundId}': {
    get: operation('정산', '회차·지출·참여 내역 조회', { response: ref('RoundDetail'), parameters: pageParameters, description: '같은 DB 스냅샷의 원본과 버전을 반환합니다. 최종화 뒤에는 전체 참여자의 송금 관계와 금액을 포함하지만 계좌정보는 반환하지 않습니다. 제외·모임 이탈 후에도 참여 이력으로 조회할 수 있습니다.' }),
