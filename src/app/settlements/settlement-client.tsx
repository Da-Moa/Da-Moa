...
export default function SettlementClient({ roundId }: { roundId: string }) {
  ...
  return <>
    ...
    {!data ? settlement.loading && <Loading /> : <div className="stack">
      <section className="tab-heading compact"><p>{data.groupName} · {data.currency}</p><h1>{data.name}</h1><div className="heading-status"><StatusBadge status={data.status} /><button className="text-button" disabled={settlement.loading} onClick={() => void reload()} type="button">{settlement.loading ? '확인 중…' : '최신 정보 새로고침'}</button></div></section>
      {!data.finalized ? <section className="domain-card stack"><h2>최종 금액을 준비하고 있어요</h2>{data.status === 'LOCKED' ? <><p>나누어떨어지지 않은 금액이 있어요. 생성자가 한 번 추첨하면 최종 부담액과 보낼 금액이 정해져요.</p><p className="help-text">{data.currency === 'USD' ? '1센트' : data.currency === 'JPY' ? '1엔' : '1원'}씩 추가 부담할 사람을 지출별로 뽑아요. 결과는 한 번 저장되며 다시 뽑을 수 없어요.</p>{data.isCreator ? <button className="primary-button" disabled={action.busy} onClick={() => void command('draw')} type="button">{action.busy ? '결과 저장 중…' : '랜덤 돌리기'}</button> : <p className="notice">생성자의 추첨을 기다려 주세요.</p>}</> : <p>아직 전송 전이에요. 생성자가 정산을 확정하고 전송해야 최종 안내를 볼 수 있어요.</p>}</section> : <>
        ...
        {data.outgoing.length > 0 && <section className="stack"><h2 className="section-heading">이 사람에게 보내 주세요</h2>{data.outgoing.map(transfer => <article className="domain-card stack" key={transfer.receiverId}><div className="row-between"><h3>{transfer.displayName}</h3><strong className="money">{formatMoney(transfer.amountMinor, data.currency)}</strong></div>
        ...
        {data.incoming.length > 0 && <section className="domain-card stack"><h2>이 사람에게 받아요</h2><ul className="member-list">{data.incoming.map(transfer => <li key={transfer.senderId}><span>{transfer.displayName}</span><strong className="money">{formatMoney(transfer.amountMinor, data.currency)}</strong></li>)}</ul><p className="help-text">입금 여부는 직접 확인해 주세요.</p></section>}
        {data.outgoing.length === 0 && data.incoming.length === 0 && <p className="notice">주고받을 금액이 없어요. 회차 종료는 생성자가 별도로 처리해요.</p>}
        ...
        {data.status === 'COMPLETED' && <p className="notice">생성자가 정산 종료를 표시했어요. 지출·금액은 변경할 수 없으며, 수취 계좌는 현재 등록 정보를 표시해요.</p>}
      </>}
