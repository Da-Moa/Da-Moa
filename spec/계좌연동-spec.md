...
작성 기준: 2026-09-10. [계좌연동-intent.md](../intent/계좌연동-intent.md), 대표 계좌 교체 시 금융결제원 연결 유지·회원탈퇴 시에만 연결 해제를 적용한다는 최신 결정, 현재 저장소의 구현을 기준으로 한다. **구현을 위한 명세이며 기능 구현 완료를 의미하지 않는다.**
...
```text
신규 가입·재가입:
카카오 OAuth → 금융결제원 사용자 OAuth → 계좌정보 입력
→ 계좌실명조회 → 예금주 일치 확인 → 계좌 저장·온보딩 완료

기존 회원의 대표 계좌 변경:
기존 금융결제원 연결 재사용 → 새 계좌정보 입력 → 계좌실명조회 → 대표 계좌 교체
미연결·재동의 필요 시에만 금융결제원 사용자 OAuth 추가
```

- 입력값은 은행, 전체 계좌번호, 생년월일, 예금주명이다.
- 대표 계좌 변경은 금융결제원 연결·기존 계좌 등록·동의를 해제하지 않는다. 변경 자체를 이유로 사용자 OAuth를 반복하거나 진행 중 정산에 따른 제한을 적용하지 않는다.
- **다모아가 요청하는 외부 연결 해제는 회원탈퇴에서만 수행한다.** 기존 미종료 참여 회차에 따른 탈퇴 제한은 유지한다.
- 외부에서 동의가 철회되거나 토큰 갱신이 불가능한 경우의 재인증 요구는 대표 계좌 교체에 따른 연결 해제와 구분한다.
- 계좌실명조회는 입력한 계좌·인증정보·예금주 일치를 확인한다. 입력 계좌와 OAuth 등록계좌의 동일성, 로그인한 사람의 소유권, 실제 송금·입금 성공을 보장한 것으로 표시하지 않는다.
- 등록계좌 목록 선택, 잔액·거래내역 화면, 이체 실행·입금 자동 확인은 이번 범위에 없다. 기존 정산 금액·수취 계좌 공개 범위·실시간 반영 정책은 유지한다.

[정산기능-spec.md](./정산기능-spec.md)의 수기 계좌 저장·계좌 검증 제외 조항은 이 명세로 대체한다. 정산 기능의 다른 정책은 변경하지 않는다.
...
| 계좌 확인·저장 | 별도 검증 완료 토큰 API 없이 기존 저장 요청 안에서 서버 실명조회 후 저장 |
| 기존 회원 전환 | 다음 계좌 저장 때 적용. 기존 계좌를 확인 완료로 일괄 전환하지 않음 |
| 생년월일 | 요청·화면 메모리에서만 사용. DB·로그·브라우저 영구 저장소에 원문 보관하지 않음 |
...
| 현재 파일 | 현재 동작 | 필요한 변경 |
|---|---|---|
| [auth-store.ts](../src/lib/auth-store.ts) | 수기 계좌 저장, 가입 완료·세션 발급, 조건부 소프트 탈퇴 | 금융결제원 인증 조건, 확인한 계좌만 저장, 버전 검사, 탈퇴 해제 대기 상태 연결 |
| [auth.ts](../src/lib/auth.ts) | 카카오 인증·서비스 쿠키·안전한 복귀 경로 | 기존 서비스 인증 재사용. 금융결제원 state와 카카오 state·쿠키는 분리 |
| [onboarding-client.tsx](../src/app/onboarding/onboarding-client.tsx) | 계좌 세 필드로 바로 가입 완료 | 금융결제원 인증 선행 화면, 생년월일, 확인 중·실패·완료 상태 |
| [ui.tsx](../src/app/home/ui.tsx) | `BankFields`, `AccountPanel`, 계좌 저장·탈퇴·계정 모달 | 은행 선택·생년월일, 연결 재사용, OAuth 복귀 시 계정 모달 재개 |
| [온보딩 API](../src/app/api/me/onboarding/route.ts) | POST, Origin 검사, 16 KiB JSON, `{data:{id,returnTo}}` 및 세션 쿠키 | 외부 실명조회 후 가입 트랜잭션, 성공 응답 유실 복구 |
...
| ID | 요구사항 |
|---|---|
| OB-01 | 신규·재가입 회원은 카카오 인증, 금융결제원 사용자 인증, 입력 계좌 확인·저장을 모두 완료해야 일반 서비스 세션을 받는다. |
| OB-02 | 유효한 기존 금융결제원 연결은 대표 계좌 교체 때 재사용하며 외부 해제 요청을 전혀 보내지 않는다. |
| OB-03 | 미연결·재동의 필요 상태만 금융결제원 사용자 OAuth를 시작한다. 접근 토큰만 만료됐고 갱신 가능하면 서버에서 갱신한다. |
| OB-04 | 은행·계좌번호·생년월일을 서버에서 검증하고 계좌실명조회에 전달한다. 입력 예금주명은 응답과 서버에서 비교한다. |
| OB-05 | API와 참가은행의 정상 응답, 요청 계좌와의 일치, 예금주 일치를 모두 통과한 값만 저장한다. |
| OB-06 | 인증 취소·입력 오류·실명조회 실패·저장 실패 시 기존 계좌와 가입 상태를 보존한다. |
| OB-07 | 저장 직전에 세션·회원·연결·계좌 버전을 다시 검사한다. 탈퇴·다른 계좌 변경에 뒤늦은 요청이 적용되지 않는다. |
...
`CONNECTED`는 사용자 인증 연결의 상태이며 정산용 계좌와 OAuth 등록계좌의 동일성을 뜻하지 않는다. 대표 계좌를 A에서 B로 바꿔도 연결 상태·연결 ID는 그대로다. 정상 토큰 갱신은 연결 해제가 아니다.
...
- 계좌 저장 때 계좌 정보·확인 시각·거래 식별자를 함께 바꾸고 `bank_version`을 1 증가시킨다.
...
| 메서드·경로 | 인증·요청 | 응답·동작 |
|---|---|---|
| GET `/api/me` | 기존 app/onboarding 세션 | 기존 응답에 `bankVersion`, 계좌 `bankCode/verifiedAt`, `openBanking:{status,authenticatedAt}` 추가 |
...
- `birthDate`는 내부 API의 날짜 형식이다. 금융결제원 전송 형식은 확인된 인증정보 유형에 따라 서버에서 변환한다.
- 알 수 없는 필드·계좌 소유자 ID·`verified`·`fintechUseNum`으로 검증이나 연결 대상을 바꿀 수 없다.
- API 응답은 생년월일, 금융결제원 사용자 식별자, 토큰, 검증 대상과 무관한 계좌정보를 반환하지 않는다.
...
| 409 `openbanking_required` | 금융결제원 연결 없음 |
| 409 `openbanking_reauth_required` | 사용자 재동의 필요. 금융결제원 인증 안내 |
...
공급자 오류가 인증정보 불일치인지 장애인지 구분되지 않으면 확인 성공으로 처리하지 않는다. 내부 설정 오류를 사용자 생년월일 오류로 표시하지 않는다.
...
- 사용자 인증 요청 URL·필수 파라미터·scope·토큰 교환·갱신 규격은 승인된 상세 명세로 확정한다. 카카오의 PKCE·nonce 설정을 지원 여부 확인 없이 복사하지 않는다.
...
대상은 `POST /v2.0/inquiry/real_name`이며, 환경별 승인된 API 호스트를 사용한다. 주요 매핑은 다음과 같다. [계좌실명조회 명세](https://developers.kftc.or.kr/dev/openapi/open-banking/account)

| 다모아 입력·서버 값 | 금융결제원 요청 |
|---|---|
| 지원 목록에서 확인한 `bankCode` | `bank_code_std` |
| 정규화한 `accountNumber` | `account_num` |
| `birthDate`에서 변환한 인증정보 | `account_holder_info` |
| 서버가 정한 인증정보 유형 | `account_holder_info_type` |
...
- 예금주명은 NFC와 앞뒤 공백 정리 후 1~100자로 제한한다. 서버에서 응답과 같은 방식으로 비교한다.
...
2. 유효한 연결이 있으면 이를 재사용한다. 새 인증이 필요하면 32바이트 난수 state를 만들고 SHA-256 해시와 회원·시작 세션·복귀 경로를 저장한다. 수명은 10분이며 세션 만료를 넘기지 않는다.
...
1. Origin·세션·입력을 검증하고 멱등 키의 기존 성공 결과를 확인한다. 같은 성공 요청이면 외부 API를 다시 호출하지 않고 결과를 반환한다.
2. 연결을 확인·필요 시 갱신하고, 짧은 트랜잭션에서 현재 회원·세션·연결 ID·`bank_version`·호출 제한을 확인한다.
3. 트랜잭션 밖에서 이용기관 토큰을 준비하고 실명조회를 수행한다. 요청 입력은 이 작업 동안 고정한다.
4. 최종 쓰기 트랜잭션에서 멱등 키를 다시 검사하고 회원·세션·연결·계좌 버전을 재검증한다.
5. 확인된 계좌와 확인 정보·새 버전, 멱등 성공 메타데이터를 함께 저장한다. 금융결제원 연결·토큰·동의는 계좌 변경 대상에서 제외한다.
6. 커밋 후 기존 계좌 변경 알림을 발행하고 응답한다. 실시간 발행 실패가 이미 저장된 계좌를 실패로 바꾸지 않는다.

`bank_updated_at`은 현재 초 단위이므로 같은 초의 변경을 구분하지 못한다. 버전이 달라지면 자동으로 새 계좌를 덮어쓰지 않고 409로 최신 확인을 요구한다.

멱등 요청에는 생년월일이 포함되므로 기존 일반 SHA-256 본문 지문을 그대로 사용하지 않는다. 계좌 저장 경로에서 정규 입력·메서드·사용자 ID의 **서버 비밀키 기반 HMAC**을 만들고 그 지문만 기존 `replayMutation`에 전달한다. 동일 키·다른 입력은 충돌로 처리하고, DB에는 생년월일 원문·복호화 가능한 사본을 저장하지 않는다. 성공 메타데이터는 회원 ID와 저장 버전만 보관한다.

### 7.3 온보딩 POST

- 실명조회 전후 검증 순서는 계좌 PUT과 같다. 최종 트랜잭션은 기존 `completeOnboarding`의 회원 복구·가입 완료·세션 교체를 재사용한다.
...
- 작은 운영 스크립트 `scripts/retry-openbanking-disconnect.mjs`가 처리 시각이 지난 대기 연결을 조회해 같은 해제 함수를 호출하도록 한다. 운영 스케줄러에서 주기적으로 실행하며 별도 큐 프레임워크를 도입하지 않는다.
...
탈퇴 계정의 카카오 재로그인은 기존처럼 onboarding 세션을 발급한다. 이전 연결이 `DISCONNECT_PENDING`이면 정리 중 상태를 안내하고 새 금융결제원 OAuth와 재가입 완료를 보류한다. 상태 재조회로 해제 완료를 확인한 뒤 새 OAuth를 시작한다.
...
- 온보딩에서 금융결제원 인증 전에는 ‘계좌 연결하기’를 제공하고 입력·가입 완료를 서버 상태로 제한한다. 인증 후 네 필드를 입력한다.
- 은행은 공통 지원 목록의 native select, 생년월일은 native date 입력을 사용한다. 예금주명과 계좌번호에 명확한 label을 제공하고 키보드로 전체 흐름을 완료할 수 있게 한다.
- 설정의 유효 연결에서는 즉시 계좌 입력·확인을 진행한다. 재연결이 필요한 상태에서만 금융결제원 화면으로 이동한다.
- 새 대표 계좌 저장 전까지 기존 계좌와 확인 시각을 유지한다. 저장 중 중복 제출을 막고 ‘계좌 확인 중’ 상태를 알린다.
- 계좌·예금주 오류는 입력을 유지하고 해당 필드로 이동할 수 있게 한다. 공급자가 반환한 다른 예금주명·인증정보를 오류 화면에 노출하지 않는다.
- 생년월일은 OAuth 리디렉션 전에 수집하지 않는다. 페이지 이동·로그아웃·모달 닫기·명시적 작업 포기 때 폼과 미완료 요청 메모리를 정리한다.
- 응답 유실 후 같은 요청을 재전송할 때만 현재 화면 메모리의 생년월일을 재사용한다. 화면을 떠난 후에는 저장 결과를 조회하고, 미저장이면 새 입력으로 다시 확인한다.
- 기존 확인 없는 계좌는 ‘확인 전’, 새 저장 성공은 ‘계좌 확인 완료’로 표시한다. OAuth 등록계좌와의 동일성·실제 입금 완료를 암시하는 문구는 사용하지 않는다.
- 탈퇴 대기 안내에는 연결 정리 여부와 재시도 안내만 표시하고 내부 토큰·DB·작업 큐 정보를 노출하지 않는다.
...
추가 이용기관 식별자·거래번호 구성·은행 목록·인증정보 유형은 공식 명세 확인 후 설정 또는 코드 상수로 정한다. 빈 설정을 임의의 테스트 값으로 대체하거나 실제 API 오류를 성공으로 처리하지 않는다.
...
공개 계좌실명조회 안내에는 인증정보 유형과 6·7자리 예시가 혼재하므로 정확한 조합을 추측하지 않는다. 생년월일에 추가 주민등록번호 자릿수를 임의로 요구하지 않는다. [계좌실명조회 공식 문서](https://developers.kftc.or.kr/dev/openapi/open-banking/account)

서비스·API Key·콜백·테스트 데이터는 [금융결제원 시작하기](https://developers.kftc.or.kr/dev/starter/starter)에 따라 준비한다. 검증되지 않은 외부 URL·필드를 확정된 API 계약으로 구현하지 않는다.
...
3. 기존 저장 API에 외부 확인과 최종 트랜잭션 검사를 적용하고 `/api/me`·OpenAPI 계약을 확장한다.
4. 온보딩·설정 UI를 새 계약으로 동시에 전환한다. 배포 후 구형 입력 요청이 검증 없이 통과하는 경로를 남기지 않는다.
...
| AT-02 | 금융결제원 인증 전 저장 직접 호출 | 외부 실명조회·계좌 저장·가입 완료 모두 차단 |
...
| AT-07 | 같은 키 성공 재시도, 같은 키 다른 입력 | 외부 재조회 없이 기존 결과 반환 또는 멱등 충돌 |
...
| AT-14 | 해제 대기 중 카카오 재로그인·금융결제원 OAuth | 카카오 로그인은 가능, 이전 해제 완료까지 새 외부 연결 보류 |
