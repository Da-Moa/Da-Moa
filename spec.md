...
OCR, 계좌 검증 API, 금융기관을 통한 실제 송금·입금 자동 검증, 외부 메시지 발송, 환불, 복수 결제자, 비율 분배, 환전, 회차 간 이월·상계, 관리자의 개별 모임 멤버 제거 UI·API는 포함하지 않는다. ‘전송’은 앱 안에서 회차를 잠그는 명령이다.
...
| 항목 | 기본 설계 |
|---|---|
| 기록 시작 | 모든 활성 모임 참여자가 멤버를 고른 뒤 ‘이 멤버로 기록 시작’을 누르면 회차를 `RECORDING`으로 생성. 요청자가 회차 생성자이자 필수 참여자이며 준비 화면을 위한 별도 DB 상태는 두지 않음 |
| 추첨 실행자 | 해당 회차를 시작하고 관리하는 회차 생성자. 다른 참여자는 결과 조회만 가능 |
| 정산 종료 시점 | 잠금·최종 금액 저장 후 모든 수취 건이 확인되면 일반 종료. 회차 생성자는 미확인 건 경고 후 강제 종료 가능 |
| 회차 사용자 제외 | 해당 회차의 `round_members.excluded_at`만 설정하고 `ALL` 분배를 재계산. 현재 모임 멤버십과 다음 회차 후보는 유지 |
...
| FR-05 | 기록 단계에서 해당 회차의 제외되지 않은 참여자가 지출을 작성한다. 작성자 또는 해당 회차 생성자만 수정·삭제한다. 모임 생성자라는 이유만으로 다른 사람이 작성한 지출을 바꿀 수 없다. |
...
| FR-11 | 회차 생성자가 현재 기록 단계인 회차를 하드 삭제할 수 있다. 이전에 확정했어도 재오픈했다면 취소 가능하다. |
...
| FR-16 | 정산 안내는 금액과 사용자가 직접 표시한 완료 상태를 관리하며 금융기관의 실제 이체 성공을 자동 검증하지 않는다. 이전 회차의 금액은 그 회차에 남긴다. |
| FR-17 | 일반 참여자는 해당 모임에서 자신이 제외되지 않은 미종료 회차에 참여 중이면 모임에서 나갈 수 없다. 차단 회차가 없으면 `group_members.left_at`을 설정하며 기존 `round_members`와 과거 조회 권한은 유지한다. |
| FR-18 | 모임 생성자는 일반 나가기를 사용할 수 없다. 모임 생성자의 해당 회차 참여 여부와 관계없이 모임 전체에 `status != COMPLETED`인 회차가 하나라도 있으면 모임 없애기를 거부한다. 회차가 없거나 모든 회차가 완료됐으면 모임 행·회차·과거 정산은 보존하고 모든 활성 멤버십과 초대를 종료해 현재 모임을 닫는다. |
| FR-19 | 모임 상세의 현재 멤버는 모임 생성자를 첫 번째로, 일반 참여자를 그 뒤에 표시한다. 요약은 최대 4명이며 전체 인원이 5명 이상이면 ‘더보기’ 버튼으로 전체 목록 모달을 연다. |
| FR-20 | 한 모임은 생성자를 포함해 활성 멤버 최대 10명이다. 정원이 찬 뒤 새 참여와 이탈자의 재참여를 거부하고 기존 활성 멤버의 중복 수락은 허용한다. 멤버 이탈로 빈자리가 생기면 다시 참여할 수 있다. |
| FR-21 | 모임 상세의 본인 참여 회차는 최신순으로 처음 3개만 표시한다. 4개 이상이면 ‘더보기’를 눌러 전체 회차를 별도 모달에서 확인하며 홈·전체 정산 기록 목록에는 이 축약을 적용하지 않는다. 제목 옆에는 검색 버튼을 두지 않고 검색·상태 조건은 모달 목록에만 적용해 본문의 최신 3개를 유지한다. 모달이 열린 동안 문서 스크롤을 잠그고 모달 끝의 스크롤이 뒤 페이지로 전달되지 않게 한다. |
| FR-22 | 정산 기록의 안내 문구 아래와 모임 상세의 본인 참여 회차 더보기 모달에는 돋보기 아이콘을 내부에 포함한 검색창을 표시하고 그 아래에 전체·진행 중·정산 종료 상태 버튼을 나열한다. 검색어는 공백 제거 후 1~100자이며 모임명 또는 회차명에 포함되는 본인 참여 회차만 대소문자 구분 없이 최신순으로 조회한다. 검색은 상태·모임·커서 조건과 함께 적용한다. |

### 3.1 서버 권한 검사

`현재 모임 생성자`는 **활성 계정 + 활성 모임 멤버십 + `groups.creator_id` 일치**를 모두 만족한다. 이 역할에만 주는 고유 권한은 초대·모임 닫기 같은 모임 관리다. 모임 생성자도 활성 모임 참여자 자격으로 회차를 만들 수 있지만, 다른 사람이 만든 회차에서는 회차 생성자나 지출 작성자가 아닌 한 회차 관리 권한이 없다. 해당 회차에서 제외돼도 모임 멤버십과 모임 관리 권한은 유지된다. `현재 회차 생성자`는 **활성 계정 + `rounds.creator_id` 일치 + 해당 회차의 제외되지 않은 `round_members` 관계**를 만족하며 해당 회차의 수명주기와 전체 지출 수정 권한을 가진다. 클라이언트가 보낸 역할·작성자·계좌 소유자 ID는 권한 근거로 사용하지 않는다.

| 작업 | 추가 조건 |
|---|---|
| 모임 상세·멤버 후보 조회 | 현재 활성 모임 멤버. 과거 참여자에게 모임 전체 자료를 열어 주지 않음 |
| 회차 상세·증빙·본인 정산 조회 | 해당 `round_members` 행 존재. 이탈·회차 제외 후에도 조회 가능 |
| 회차 생성 | 현재 활성 모임 멤버. 요청자 자신을 포함한 활성 후보 최소 2명 |
| 지출 작성 | `RECORDING`, `round_members.excluded_at IS NULL` |
| 지출 수정·삭제·증빙 변경 | `RECORDING`, 제외되지 않은 원작성자 또는 현재 회차 생성자 |
| 확정·재오픈·제외·취소·잠금·추첨·종료 | 현재 회차 생성자이며 각 상태 조건 충족 |
| 초대 발급·폐기 | 현재 모임 생성자 |
| 모임 나가기 | 활성 일반 멤버. 같은 모임에 본인이 제외되지 않은 미종료 참여 회차가 없어야 함 |
| 모임 없애기 | 현재 모임 생성자. 일반 나가기 대신 사용하며 모임 전체에 미종료 회차가 없어야 함. 모임 생성자의 회차 참여 여부는 조건에 포함하지 않음 |
| 계좌 수정 | 본인 계정만 가능. 완료 회차의 수취 계좌가 최신으로 보이는 것은 원본 변경이 아님 |

한 회차에서 제외돼도 현재 모임 멤버십과 다른 회차의 참여 자격은 그대로 유지한다. 각 회차의 기록 권한은 그 회차 관계로 판정하며 모임 관리 권한에는 현재 모임 멤버십이 필요하다. 일반 참여자의 자진 나가기 제한은 `excluded_at IS NULL`인 미종료 회차만 대상으로 하지만, 회원탈퇴 제한은 제외 여부와 관계없이 모든 미종료 참여 회차를 대상으로 한다.
...
```mermaid
stateDiagram-v2
    [*] --> RECORDING: 활성 모임 참여자가 시작 · 회차 생성자 지정
    RECORDING --> CONFIRMED: 확정
    CONFIRMED --> RECORDING: 재오픈
    CONFIRMED --> LOCKED: 전송
    LOCKED --> LOCKED: 나머지 1회 배분 및 최종 저장
    LOCKED --> LOCKED: 수취인의 송금자별 입금 확인·해제
    LOCKED --> COMPLETED: 모든 수취 건 확인 후 일반 종료 또는 생성자 강제 종료
...
| 받은 정산 확인 | `LOCKED`, `finalized_at IS NOT NULL`, 본인이 해당 송금의 수취인 | `senderId`가 있으면 본인에게 보내는 해당 송금 행 하나, 없으면 본인의 수취 행 전체에 `received_at`을 설정 또는 해제. 회차 버전은 변경하지 않음 |
| 정산 종료 | `LOCKED`, `finalized_at IS NOT NULL`, 모든 송금 행의 수취 확인 완료 | 확인 상태를 같은 트랜잭션에서 재검사하고 `COMPLETED`, `completed_at` 저장 |
| 강제 정산 종료 | `LOCKED`, `finalized_at IS NOT NULL`, 회차 생성자 경고 확인 | 미확인 수취 건이 있어도 `COMPLETED`, `completed_at` 저장. 미확인 상태는 그대로 보존 |
...
- 금액은 통화 최소 단위의 `NUMERIC` 정수로 저장하고 PostgreSQL 조회 시 문자열로 읽는다. 컬럼 정밀도로 금액을 잘라내지 않고, 애플리케이션 트랜잭션에서 지출 한 건과 회차 합계의 상품 한도를 검증한다.
...
- 모임 닫기는 모임 생성자의 회차 참여 여부를 보지 않고 `rounds.group_id`로 모임 전체의 `status <> 'COMPLETED'` 존재 여부를 같은 트랜잭션에서 다시 검사한다. 미종료 회차가 없으면 모임·회차 행을 유지한 채 모든 활성 멤버십의 `left_at`과 모든 활성 초대의 `revoked_at`을 설정한다. `rounds.group_id`는 그대로 보존하여 완료 회차·정산 조회가 끊기지 않게 한다.
...
API 입력은 쉼표·지수 표기·기호가 없는 십진 문자열이다. KRW·JPY는 소수점 없는 정수, USD는 소수점 이하 최대 2자리만 허용한다. JSON 숫자, 빈 값, NaN/Infinity, 0·음수는 거부한다. UI는 `type="text"`, 적절한 `inputMode`를 사용하고 쉼표를 표시용으로만 넣으며 큰 금액을 `Number()`로 바꾸지 않는다.

지출 한 건의 최대값은 KRW·JPY `100,000,000`, USD `100,000,000.00`이다. 회차 전체 지출 합계의 최대값은 KRW·JPY `1,000,000,000`, USD `1,000,000,000.00`이다. 생성은 현재 합계에 새 금액을 더하고, 수정은 현재 합계에서 기존 금액을 뺀 뒤 변경 금액을 더해 검증한다. 프론트 입력 상한은 `min(지출 한도, 회차 한도 - 현재 합계 + 수정 전 금액)`으로 계산하고 초과 입력을 그 값으로 즉시 조정한다. 합계 확인과 저장은 같은 쓰기 트랜잭션에서 다시 처리한다.
...
1. 회차 생성자의 전송을 커밋한 뒤 안내 화면에 진입한다. 나머지가 있는 회차는 ‘랜덤 돌리기’를 제공하고 개인별 최종 송금액·복사 링크를 아직 제공하지 않는다.
...
회차 변경 요청에는 `expectedVersion`을 요구한다. 서버 버전과 다르면 `409 stale_round`와 현재 버전을 반환한다. 지출 생성·수정 폼은 최신 회차를 자동 조회한 뒤 캡처한 입력을 최신 버전으로 한 번만 재저장하고 첫 충돌 카드는 표시하지 않는다. 재저장도 충돌하거나 회차 상태가 바뀌면 입력을 유지하고 오류를 표시한다. 그 밖의 명령은 최신 자료를 다시 읽고 사용자가 재시도한다.
...
- 이미 최종 저장된 회차의 추첨을 다른 키로 요청해도 다시 뽑지 않고 기존 결과를 안내한다. 이 경우 현재 회차 생성자 권한 검사 후 `finalized_at`을 `expectedVersion`보다 먼저 확인한다. 취소의 같은 키 재시도는 회차가 없어도 기존 취소 성공 결과를 반환한다.
...
## 8. 회차 사용자 제외·모임 이탈·과거 조회

확정 화면에서 제외를 시도하면 서버는 먼저 관련 지출을 검사하여 차단 사유를 반환할 수 있다. 이 검사는 데이터를 변경하지 않는다. 실제 제외 명령은 회차 생성자가 재오픈한 **기록 단계**에서만 성공한다.
...
회차 제외는 모임 나가기나 모임 멤버 제거가 아니다. 제외된 사용자와 제외된 모임 생성자의 활성 모임 멤버십·모임 권한·다음 회차 후보 자격은 유지한다. 모임 나가기는 아래의 기존 자진 나가기 흐름으로만 처리한다. 관리자가 다른 모임 멤버 한 명을 제거하는 정책과 UI·API 계약은 아직 없으므로 이 명세와 API 목록에 추가하지 않는다.

### 8.1 일반 참여자의 자진 나가기

일반 참여자에게 모임 나가기를 제공한다. 같은 쓰기 트랜잭션에서 활성 일반 멤버 여부와 아래 차단 조건을 검사한다.

```sql
EXISTS (
  SELECT 1
  FROM rounds r
  JOIN round_members rm ON rm.round_id = r.id
  WHERE r.group_id = :groupId
    AND rm.user_id = :userId
    AND rm.excluded_at IS NULL
    AND r.status <> 'COMPLETED'
)
```

조건에 맞는 회차가 하나라도 있으면 나가기를 거부한다. 완료 회차만 있거나 미종료 회차에서 이미 제외된 사용자는 나갈 수 있다. 성공하면 `group_members.left_at`만 설정하며 `round_members`, 지출·증빙·분담·정산 결과는 변경하지 않는다. 이탈 효과는 다른 기존 회차로 전파하지 않고, `round_members`에 근거한 과거 조회를 유지한다. 신규 후보는 활성 계정·가입 완료·활성 모임 멤버십을 모두 만족하는 사용자로 조회한다.

### 8.2 모임 생성자의 모임 없애기(닫기)

모임 생성자는 일반 나가기를 사용할 수 없으며 관리 권한도 이전하지 않는다. 모임 없애기 요청은 현재 활성 모임 생성자 권한을 확인한 뒤, 모임 생성자가 각 회차에 참여했는지와 무관하게 아래 조건을 같은 트랜잭션에서 검사한다.
...
조건에 맞는 회차가 하나라도 있으면 `409 unfinished_group_rounds`로 거부한다. 회차가 없거나 모든 회차가 `COMPLETED`이면 같은 시각을 사용해 모든 `group_members WHERE left_at IS NULL`에 `left_at`을 설정하고, 모든 미폐기 `group_invites`에 `revoked_at`을 설정한다. `groups` 행과 모든 `rounds`·지출·정산 이력은 삭제하거나 변경하지 않는다. 모임은 활성 멤버와 유효한 초대가 없는 닫힌 상태가 되고, 과거 참여자는 `round_members`를 근거로 완료 회차를 계속 조회한다. 성공 응답은 기존 변경 응답처럼 `{ data: { id } }`이며 물리 삭제를 뜻하지 않는다.

모임 닫기 성공과 `mutation_requests` 기록도 같은 커밋에 넣는다. 응답 유실 뒤 동일한 모임 생성자가 같은 `Idempotency-Key`와 요청 내용으로 재시도하면, 이미 모든 멤버십이 종료됐더라도 인증한 사용자와 성공 기록을 먼저 대조해 같은 성공 응답을 반환한다. 다른 키로 닫힌 모임을 다시 요청하면 활성 모임 생성자 권한이 없으므로 거부한다.

나가기·모임 닫기는 회차 생성·초대 발급·초대 수락과 같은 쓰기 잠금 아래 처리한다. 어떤 활성 모임 참여자든 미종료 회차 생성을 먼저 성공시키면 닫기를 거부한다. 닫기가 먼저 성공하면 이후 회차 생성은 활성 모임 멤버십 조건에서, 초대 발급은 활성 모임 생성자 조건에서, 초대 수락은 활성 초대·모임 조건에서 거부한다. 초대 수락이 먼저 성공했더라도 미종료 회차가 없다면 뒤이은 닫기가 새 멤버를 포함한 모든 활성 멤버십과 초대를 종료한다. 완료 회차만 존재하는 경우에도 닫기를 허용한다.
...
멤버십은 가입 완료 시 복구하지 않는다. 과거 조회는 같은 ID로 가능하고 모임 재참여는 모임 생성자가 제공한 유효한 초대로 새로 수락해야 한다. 모임·회차 생성자도 동일하게 적용한다. 관리 권한 이전·생성자 부재 시 자동 복구 기능은 없다.

이 정책 조합의 결과로, 모임 생성자가 탈퇴하면 새 초대 발급·폐기와 모임 닫기를 수행할 관리자가 없어진다. 남아 있는 활성 모임 참여자는 계속 새 회차를 만들고 자신이 만든 회차를 관리할 수 있다. 원모임 생성자도 재가입만으로 그 모임의 관리 권한을 되찾지 않는다. 현재 범위에 모임 관리 복구 경로는 없으며 새 모임을 만들 수 있다. 이 제한을 탈퇴 안내에 표시한다.
...
탈퇴 가능하면 `deleted_at` 설정, 모든 세션 폐기, 모든 활성 모임 멤버십 이탈을 함께 처리한다. 사용자 식별자·계좌·과거 참가자 이름·작성·결제·부담·정산 관계는 유지한다. 수취인의 탈퇴를 이유로 과거 정산 조회에서 그 사람을 JOIN으로 누락시키지 않되 프로필 이미지 URL은 `null`로 마스킹하여 기본 프로필로 표시한다. 탈퇴된 사용자에게 일반 앱 접근이나 신규 회차 참여는 허용하지 않는다.

회원탈퇴의 차단 범위는 일반 참여자의 특정 모임 나가기보다 넓다. 회원탈퇴는 `round_members.excluded_at`과 관계없이 참여한 미종료 회차가 하나라도 있으면 계속 거부한다. 모임 생성자의 일반 나가기 금지·모임 닫기 규칙도 회원탈퇴로 인한 계정 소프트 삭제와는 별도로 적용한다.
...
| `/home/groups/[groupId]` | 생성자 우선 현재 멤버 요약(최대 4명)과 5명 이상일 때의 전체 목록 모달, 본인 참여 회차 최신 3개와 4개 이상일 때의 검색·상태 필터가 있는 전체 목록 모달. 모든 활성 참여자에게 본인을 필수로 포함하는 회차 생성 폼을 제공한다. 모임 생성자에게만 초대·모임 없애기를, 다른 참여자에게는 조건부 모임 나가기를 제공 |
| `/home/rounds/[roundId]` | 기록 목록·작성·증빙, 카카오 프로필 이미지가 포함된 참여자 노드와 지출 변경에 따라 갱신되는 본인 기준 예상·최종 송금 관계도, 전체 지출·관계별 금액의 자릿수별 증감 애니메이션, 권한별 수정·삭제·제외, 확정·재오픈·취소·전송 |
| `/home/history` | 참여 이력으로 조회한 회차 목록. 모임명·회차명 검색과 상태 필터를 함께 제공하며 현재 멤버십 없는 과거 회차도 포함 |
...
| `/settlements/[roundId]` | 로그인한 본인의 안내. 잠금 후 추첨 대기 또는 상대 이름·프로필 이미지·최종 금액·필요한 최신 계좌·링크 복사. ‘이 사람에게 받아요’의 송금자 행을 눌러 개별 입금 확인·해제하고 카드 안의 ‘모두 정산 확인’ 체크박스로 본인의 수취 건 전체를 변경한다. 확인한 이름·금액은 취소선 처리하고 남은 수취 금액에서 차감한다. 실시간 게이지와 별도 수취인별 완료 현황을 표시하며, 완료 뒤에는 읽기 전용이다. 회차 생성자는 모든 수취 건 확인 후 종료하거나 경고 후 강제 종료한다. |

전송 버튼 앞에는 ‘이대로 사용자들에게 메시지를 전송할까요?’를 표시하고, ‘전송하면 기록이 잠기며 다음 화면에서 링크를 복사해 직접 공유합니다’를 함께 안내한다. 실제 메시지 발송 성공이라고 표시하지 않는다.

버튼 비활성화만으로 권한을 구현하지 않는다. 서버 거부 메시지를 표시하고 중복 클릭을 억제한다. 입력 저장 실패 시 입력과 요청 키를 유지하고 재시도한다. 지출 생성·수정의 버전 충돌은 최신 자료 조회 후 같은 입력으로 한 번 자동 재저장하고, 그 밖의 상태 충돌은 현재 자료를 갱신해 오류를 표시한다.
...
| `GET /api/groups` / `POST /api/groups` | 활성 모임 목록 / `{ name }`으로 모임·모임 생성자 멤버십 생성 |
| `GET /api/groups/[groupId]` | 권한 있는 현재 멤버용 모임·멤버 후보. 멤버는 모임 생성자 우선으로 정렬 |
| `DELETE /api/groups/[groupId]` | 역할에 따라 일반 참여자의 모임 나가기 또는 모임 생성자의 모임 없애기. 참여자는 제외되지 않은 미종료 참여 회차가 있으면 거부한다. 모임 생성자는 본인 참여 여부와 무관하게 모임 전체의 미종료 회차가 있으면 거부하고, 없으면 모임·회차를 보존한 채 모든 활성 멤버십의 `left_at`과 모든 활성 초대의 `revoked_at`을 설정한다 |
| `GET /api/groups/[groupId]/rounds` | 본인 조회 권한이 있는 모임 회차 목록. 선택적 `q`로 모임명·회차명 부분 검색 |
...
| `GET /api/rounds/[roundId]/members/[userId]/exclusion-check` | 회차 생성자에게 차단 사유·관련 내역 반환. 회차 생성자 본인은 제외 불가하고 모임 생성자는 다른 참여자와 같은 조건으로 검사하며 실제 제외 시 다시 검증 |
| `POST /api/rounds/[roundId]/members/[userId]/exclude` | 기록 단계에서 `round_members.excluded_at` 설정·ALL 재분배. 모임 멤버십과 다음 회차 후보는 유지 |
...
| `POST /api/rounds/[roundId]/draw` | 회차 생성자의 회차 전체 나머지 1회 배분 |
| `POST /api/rounds/[roundId]/settlement-check` | 본인이 수취하는 송금의 입금 확인·해제. `{ checked, senderId?, expectedVersion }`이며 `senderId`가 있으면 한 건, 없으면 본인의 수취 건 전체를 변경하고 회차 버전은 올리지 않음 |
| `POST /api/rounds/[roundId]/complete` | 모든 수취 건 확인 후 회차 생성자의 정산 종료 |
| `POST /api/rounds/[roundId]/force-complete` | 미확인 수취 건이 있어도 회차 생성자가 수행하는 강제 정산 종료 |
| `DELETE /api/rounds/[roundId]` | 기록 단계 회차 하드 삭제 |
| `GET /api/rounds/[roundId]/settlement` | 본인 지급·수취 금액과 상태. `isCreator`는 조회자가 회차 생성자인지를 뜻하며 최종 저장 전에는 대기 응답만 제공 |

모든 회차 변경·명령은 별도 표시가 없어도 `expectedVersion`과 `Idempotency-Key`를 요구한다. 계좌 변경·모임 생성·나가기·모임 없애기·초대 생성/수락도 멱등 키를 사용한다. 인증 프로토콜은 기존 세션 회전 방식과 가입용 세션 검증으로 처리하며 원문 토큰을 명령 결과 테이블에 저장하지 않는다.

### 10.4 개인 안내 응답

최종 송금 행에서 `sender_id = 로그인 사용자`인 수취인만 최신 계좌를 조회한다. 수취 내역에는 지급자 이름·금액만 있으며 지급자 계좌는 필요하지 않다. 모임·회차 생성자도 같은 공개 범위를 적용한다.
...
| 409 | `stale_round`, `invalid_round_state`, `idempotency_conflict` | 최신 상태 확인. 지출 저장의 첫 `stale_round`만 같은 입력으로 1회 자동 재저장 |
...
| 409 | `group_member_limit_exceeded` | 모임은 생성자를 포함해 최대 10명임을 안내 |
...
| `src/lib/group-store.ts` 신규 | 모임 생성자 기반 초대·모임 관리, 멤버십·자진 나가기·조건부 모임 닫기·조회 |
| `src/lib/round-store.ts` 신규 | 활성 모임 멤버의 회차 생성, 회차 생성자 권한, 지출·증빙·제외·회차 명령·최종 안내 조회·멱등성 적용 |
| `src/lib/money.ts` 신규 | 통화별 문자열 파싱·최소 단위 변환·표시 |
| `src/lib/split.ts` 변경 | 몫·나머지, 최종 부담액과 잔액·송금 계산의 순수 함수 |
| `src/lib/realtime.ts`, `src/lib/realtime-server.ts` | 허용된 재조회 키, 사용자 채널, 제한 토큰 발급, 커밋 후 무효화 발행 |
...
| 2 | 모임·초대·나가기·조건부 닫기·회차 생성자·과거 조회, 로그인 목적지 | 명시적 수락, 모든 활성 멤버의 회차 생성, 시작자 필수 참여·회차 관리 권한, 모임·회차 생성자 분리, 나가기 차단, 미종료 회차가 있는 모임 닫기 차단, 완료 이력 보존 검증 통과 |
...
| 6 | 사용자별 WebSocket 무효화·재연결 복구 | 두 계정/탭의 회차 변경 자동 반영, 메시지 개인정보 비포함, 발행 실패 시 저장 유지 |
...
- 모든 활성 모임 참여자는 자신을 포함한 최소 2명으로 회차를 만들 수 있다. 선택 화면은 시작자를 첫 번째 필수 멤버로, 모임 생성자를 포함한 나머지를 일반 참여자로 그 뒤에 표시한다. 시작자를 빼거나 비활성·외부 사용자를 포함하면 거부하며 모임 생성자는 필수 참여자가 아니다. 응답의 `creatorId`·`isCreator`는 회차 생성자를, `groupCreatorId`는 모임 생성자를 정확히 구분한다.
- 작성자는 본인 지출을, 회차 생성자는 해당 회차의 모든 지출을 기록 단계에서 수정·삭제할 수 있다. 모임 생성자라도 그 회차의 생성자나 지출 작성자가 아니면 거부한다. 회차 생성자 수정 후 원작성자·결제자 정보는 보존한다.
...
- 최종 저장된 잠금 회차에서 수취인은 본인의 `incoming` 송금자 행 하나를 눌러 확인·해제하고 ‘모두 정산 확인’ 체크박스로 본인의 모든 수취 행을 함께 변경한다. 동일한 회차 버전으로 여러 수취인이 동시에 확인해도 모두 저장되며 확인 자체는 버전을 올리지 않는다. 다른 사용자의 수취 행은 변경할 수 없다.
- 확인한 송금자 행의 이름·금액은 취소선으로 표시하고 ‘내가 받을 금액’에서 정확히 차감한다. 송금자 안내의 `outgoing`은 미확인 송금만 반환해 확인된 ‘이 사람에게 보내 주세요’ 카드를 숨기고, 확인을 해제하면 양쪽 표시를 모두 복원한다. 정수 최소 단위 계산에서 반올림이나 `number` 정밀도 손실이 없어야 한다.
- 정산 안내는 모든 수취 행이 확인된 수취인 수를 게이지로 나타내고 수취인별 이름·활성 카카오 프로필·완료 여부를 별도 카드에 표시한다. WebSocket 무효화 뒤 정산 API를 다시 읽어 다른 수취인의 확인을 반영하며, 완료·강제 완료 뒤에도 저장된 상태를 읽기 전용으로 유지한다. 수취인이 없으면 게이지와 현황 카드를 숨긴다.
- 일부 수취 건만 확인한 상태의 일반 종료는 거부한다. 모든 수취 건 확인 뒤 일반 종료가 가능하고, 회차 생성자는 미확인 건이 있을 때 강제 종료할 수 있다. 송금 행이 없는 회차는 별도 확인 없이 일반 종료할 수 있으며 자동 종료하지 않는다. 완료 뒤에는 확인 변경과 재종료를 거부한다.
- 해당 회차 생성자 본인 제외, 결제자 겸 부담자 제외, 선택 분배 부담자 제외, 제외 후 최소 2명 위반을 차단한다. 모임 생성자는 다른 참여자와 같은 조건으로 제외할 수 있다. 차단 응답에 모든 관련 내역이 있고 DB 변경은 없다.
- 제외 가능한 ALL 부담자는 재분배한다. 부담자가 아닌 결제자를 제외해도 원본 결제·수취 관계가 남는다. `round_members.excluded_at`만 변경되고 모임 멤버십·다른 기존 회차·다음 회차 후보는 그대로다.
- 일반 참여자가 별도 모임 나가기를 완료하면 다음 회차 후보에서 빠지지만 기존 참여 이력 조회는 가능하다. 모임 재가입으로 이미 만들어진 회차에 자동 추가되지 않는다. 관리자에 의한 개별 모임 멤버 제거는 현재 UI·API 범위에 없다.
...
- 생성자와 일반 참여자를 합쳐 10명까지 참여할 수 있다. 10명 상태에서는 새 사용자와 과거 이탈자의 수락을 거부하고 기존 멤버의 중복 수락은 멤버 수를 바꾸지 않는다. 한 명이 나가면 대기 사용자의 수락으로 다시 10명까지 채울 수 있다.
...
- 활성 멤버 9명 상태의 서로 다른 두 사용자 동시 수락: 정확히 한 명만 성공하고 다른 요청은 `group_member_limit_exceeded`이며 커밋 후 활성 멤버는 10명이다.
- 지출 수정 ↔ 확정, 재오픈 ↔ 전송, 취소 ↔ 확정 동시 요청: 모순되는 작업이 모두 성공하지 않는다. stale version은 409다.
- 두 장치의 동시 추첨: 최종 배분은 한 번이며 사용자별 송금 결과가 일치한다.
- 참여자 확인 설정·해제 ↔ 일반·강제 종료: 공통 쓰기 트랜잭션에서 순차 처리해 일반 종료가 미확인 상태를 통과하지 못하게 한다. 강제 종료가 먼저 커밋되면 이후 확인 변경은 거부한다.
- 탈퇴 ↔ 회차 생성·초대 수락·로그인 세션 발급 동시 요청: 탈퇴 상태로 새 진행 회차에 추가되거나 유효한 일반 세션이 남지 않는다.
- 일반 참여자 나가기 ↔ 회차 생성: 회차가 먼저 생겨 참여자가 포함되면 나가기를 거부하고, 나가기가 먼저 성공하면 새 회차 후보·참여자로 선택할 수 없다.
- 모임 생성자 닫기 ↔ 임의 활성 멤버의 회차 생성: 미종료 회차 생성과 닫기가 모두 성공하지 않는다. 생성이 먼저 커밋되면 닫기는 `unfinished_group_rounds`로 거부되고, 닫기가 먼저 커밋되면 생성은 비활성 모임 멤버십으로 거부된다. 초대 수락이 먼저 성공했더라도 미종료 회차가 없으면 닫기가 새 멤버를 포함한 모든 활성 멤버십과 초대를 종료한다. 완료 회차만 있는 모임의 닫기는 성공하고 기존 행·과거 조회는 유지된다.
...
- 초대·정산 목적지가 로그인·가입·갱신을 거쳐 유지된다. 악성 `returnTo`, 만료·폐기된 초대와 활성 모임 생성자가 없는 모임의 초대 수락은 거부한다. 조회 링크로 멤버십이 생기지 않는다.
- B가 A·C에게 지급할 때 B의 HTML·RSC·API에 D·E 계좌가 없다. 모임·회차 생성자도 예외가 없다. USD·JPY에는 계좌가 없다.
- 계좌 변경 후 링크 재진입·새로고침에서 최신값, 원본 금액·추첨 결과는 동일하다. DB 조회 장애는 재시도 가능하고 계좌 조회 성공으로 표시하지 않는다.
- 회차 화면은 모든 참여자를 표시하되 송금 행은 조회자 본인이 보내거나 받는 관계만 방향·이름·금액이 있는 관계도로 표시한다. 모임·회차 생성자도 예외가 없고 다른 참여자끼리의 송금 행은 API에서도 반환하지 않는다. 추첨 전에는 각 지출의 기본 몫만 상계한 값을 ‘현재 예상’으로 구분하고 미배분 나머지를 따로 표시하며, 지출 변경 때 갱신하고 추첨 뒤 저장된 최종값으로 교체한다. 제외된 과거 참여자도 기록에서 제거하지 않는다.
- 참여자 한 명이 지출·증빙·정산 상태를 바꾸면 같은 회차를 연 다른 참여자의 화면이 WebSocket 이벤트 후 기존 API를 재조회해 반영한다. 계좌 변경도 해당 수취인에게 송금할 사용자의 열린 정산 화면에 반영한다.
- 모임 나가기·닫기 이벤트는 변경 전 활성 멤버 수신자를 확보해 발행하며, 수신 화면은 기존 GET 권한 검사를 다시 거쳐 활성 목록에서 제거하거나 현재 접근을 종료한다. 완료 회차 화면은 `round_members` 권한으로 계속 조회할 수 있다.
- WebSocket 토큰은 본인 채널 구독만 허용하고 이벤트에는 금액·계좌·지출·영수증·초대 토큰이 없다. 연결 단절 중 누락은 재연결 재조회로 복구하며 Ably 장애가 커밋된 변경을 롤백하거나 5xx로 바꾸지 않는다.
- 지출 입력 폼을 연 상태에서 실시간 변경을 받아도 폼 시작 버전으로 첫 제출해 `stale_round`를 검출한다. 최신 회차 자동 조회와 같은 입력의 1회 재저장으로 성공하면 충돌 카드를 표시하지 않고, 원격 확정이나 반복 충돌로 저장할 수 없으면 입력을 버리지 않고 오류를 표시한다.
