...
- 지속되는 모임, 초대 링크 수락, 일반 참여자의 조건부 나가기, 미종료 회차가 없는 모임의 생성자 닫기, 회차별 참여자 구성, 여러 회차 동시 진행.
...
| 생성자 모임 닫기 | 일반 나가기는 제공하지 않음. 모임 전체에 미종료 회차가 있으면 차단하고, 회차가 없거나 모두 완료됐으면 모임·회차를 보존한 채 모든 활성 멤버십과 초대를 종료 |
...
| FR-12 | 사용자 제외는 생성자·결제자 겸 부담자·특정 분배 부담자 제한과 최소 인원을 원자적으로 검사한다. 결과와 관련 내역은 팝업으로 안내하고, 선택한 지출을 펼쳐 하이라이트한다. |
...
| FR-18 | 모임 생성자는 일반 나가기를 사용할 수 없다. 생성자의 해당 회차 참여 여부와 관계없이 모임 전체에 `status != COMPLETED`인 회차가 하나라도 있으면 모임 없애기를 거부한다. 회차가 없거나 모든 회차가 완료됐으면 모임 행·회차·과거 정산은 보존하고 모든 활성 멤버십과 초대를 종료해 현재 모임을 닫는다. |
...
| 모임 없애기 | 현재 모임 생성자. 일반 나가기 대신 사용하며 모임 전체에 미종료 회차가 없어야 함. 생성자의 회차 참여 여부는 조건에 포함하지 않음 |
| 계좌 수정 | 본인 계정만 가능. 완료 회차의 수취 계좌가 최신으로 보이는 것은 원본 변경이 아님 |

다른 회차에서 제외되어 현재 모임을 이탈했더라도, 기존 회차의 제외되지 않은 참여 자격은 유지한다. 그 회차의 기록 권한도 회차 관계로 판정한다. 관리 권한에는 항상 현재 모임 멤버십이 필요하다. 일반 참여자의 자진 나가기 제한은 `excluded_at IS NULL`인 미종료 회차만 대상으로 하지만, 회원탈퇴 제한은 제외 여부와 관계없이 모든 미종료 참여 회차를 대상으로 한다.
...
- 모임 닫기는 생성자의 회차 참여 여부를 보지 않고 `rounds.group_id`로 모임 전체의 `status <> 'COMPLETED'` 존재 여부를 같은 트랜잭션에서 다시 검사한다. 미종료 회차가 없으면 모임·회차 행을 유지한 채 모든 활성 멤버십의 `left_at`과 모든 활성 초대의 `revoked_at`을 설정한다. `rounds.group_id`는 그대로 보존하여 완료 회차·정산 조회가 끊기지 않게 한다.
...
생성자는 일반 나가기를 사용할 수 없으며 관리 권한도 이전하지 않는다. 모임 없애기 요청은 현재 활성 생성자 권한을 확인한 뒤, 생성자가 각 회차에 참여했는지와 무관하게 아래 조건을 같은 트랜잭션에서 검사한다.
...
모임 닫기 성공과 `mutation_requests` 기록도 같은 커밋에 넣는다. 응답 유실 뒤 동일한 생성자가 같은 `Idempotency-Key`와 요청 내용으로 재시도하면, 이미 모든 멤버십이 종료됐더라도 인증한 사용자와 성공 기록을 먼저 대조해 같은 성공 응답을 반환한다. 다른 키로 닫힌 모임을 다시 요청하면 활성 생성자 권한이 없으므로 거부한다.

나가기·모임 닫기는 회차 생성·초대 발급·초대 수락과 같은 쓰기 잠금 아래 처리한다. 미종료 회차 생성이 먼저 성공하면 닫기를 거부한다. 닫기가 먼저 성공하면 이후 회차 생성·초대 발급은 활성 생성자 멤버십 조건에서, 초대 수락은 활성 초대·모임 조건에서 거부한다. 초대 수락이 먼저 성공했더라도 미종료 회차가 없다면 뒤이은 닫기가 새 멤버를 포함한 모든 활성 멤버십과 초대를 종료한다. 완료 회차만 존재하는 경우에도 닫기를 허용한다.
...
회원탈퇴의 차단 범위는 일반 참여자의 특정 모임 나가기보다 넓다. 회원탈퇴는 `round_members.excluded_at`과 관계없이 참여한 미종료 회차가 하나라도 있으면 계속 거부한다. 생성자의 일반 나가기 금지·모임 닫기 규칙도 회원탈퇴로 인한 계정 소프트 삭제와는 별도로 적용한다.
...
| `/home/groups/[groupId]` | 현재 멤버·회차 목록. 일반 참여자는 조건부 모임 나가기, 생성자는 초대·회차 생성과 미종료 회차가 없을 때의 모임 없애기를 제공 |
...
| `DELETE /api/groups/[groupId]` | 역할에 따라 일반 참여자의 모임 나가기 또는 생성자의 모임 없애기. 참여자는 제외되지 않은 미종료 참여 회차가 있으면 거부한다. 생성자는 본인 참여 여부와 무관하게 모임 전체의 미종료 회차가 있으면 거부하고, 없으면 모임·회차를 보존한 채 모든 활성 멤버십의 `left_at`과 모든 활성 초대의 `revoked_at`을 설정한다 |
...
| `GET /api/rounds/[roundId]` | 회차·참여자·지출·기본 분배·상태·version. 추첨 전에는 기본 몫을 상계한 예상 송금 관계·미배분 나머지 금액, 최종화 후에는 저장된 관계를 포함한다. 송금 관계는 조회자 본인이 보내거나 받는 행만 반환하며 계좌정보는 없음 |
...
| `src/lib/group-store.ts` 신규 | 모임·초대·멤버십·자진 나가기·조건부 모임 닫기·회차 생성·조회 |
...
| 2 | 모임·초대·나가기·조건부 닫기·회차·과거 조회, 로그인 목적지 | 명시적 수락, 나가기 차단, 미종료 회차가 있는 모임 닫기 차단, 완료 이력 보존, 최소 인원, 여러 회차, 이탈 후 조회 검증 통과 |
...
- 생성자의 일반 나가기는 거부한다. 생성자가 해당 회차에 참여하지 않아도 모임 전체에 기록 중·확정·잠금 회차가 하나라도 있으면 모임 없애기를 거부한다. 회차가 없거나 모든 회차가 완료됐으면 모임·회차·정산 이력을 보존하고 모든 활성 멤버십·초대를 종료한다. 과거 참여자는 완료 회차를 계속 조회할 수 있다.
...
- 생성자 모임 닫기 ↔ 회차 생성: 미종료 회차 생성과 닫기가 모두 성공하지 않는다. 생성이 먼저 커밋되면 닫기는 `unfinished_group_rounds`로 거부되고, 닫기가 먼저 커밋되면 생성은 비활성 생성자 멤버십으로 거부된다. 초대 수락이 먼저 성공했더라도 미종료 회차가 없으면 닫기가 새 멤버를 포함한 모든 활성 멤버십과 초대를 종료한다. 완료 회차만 있는 모임의 닫기는 성공하고 기존 행·과거 조회는 유지된다.
...
- 회차 화면은 모든 참여자를 표시하되 송금 행은 조회자 본인이 보내거나 받는 관계만 방향·이름·금액이 있는 관계도로 표시한다. 생성자도 예외가 없고 다른 참여자끼리의 송금 행은 API에서도 반환하지 않는다. 추첨 전에는 각 지출의 기본 몫만 상계한 값을 ‘현재 예상’으로 구분하고 미배분 나머지를 따로 표시하며, 지출 변경 때 갱신하고 추첨 뒤 저장된 최종값으로 교체한다. 제외된 과거 참여자도 기록에서 제거하지 않는다.
