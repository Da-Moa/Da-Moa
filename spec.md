...
- 회차 생성자의 정산 확정·재오픈·전송 잠금·나머지 1회 추첨·정산 종료·회차 취소.
- 조건부 회차 사용자 제외, 별도 모임 이탈 후 과거 회차 조회, 개인별 지급 안내와 링크 복사.

OCR, 계좌 검증 API, 실제 송금·입금 확인, 외부 메시지 발송, 환불, 복수 결제자, 비율 분배, 환전, 회차 간 이월·상계, 관리자의 개별 모임 멤버 제거 UI·API는 포함하지 않는다. ‘전송’은 앱 안에서 회차를 잠그는 명령이다.
...
| FR-10 | 회차 생성자가 최종 금액 저장 후 정산 종료를 표시한다. 입금 검증을 의미하지 않으며 종료 데이터는 읽기 전용이다. |
...
| `/home/rounds/[roundId]` | 기록 목록·작성·증빙, 카카오 프로필 이미지가 포함된 참여자 노드와 지출 변경에 따라 갱신되는 본인 기준 예상·최종 송금 관계도, 권한별 수정·삭제·제외, 확정·재오픈·취소·전송 |
...
| `/settlements/[roundId]` | 로그인한 본인의 안내. 잠금 후 추첨 대기 또는 상대 이름·프로필 이미지·최종 금액·필요한 최신 계좌·링크 복사 |
...
| `POST /api/rounds/[roundId]/complete` | 회차 생성자의 업무상 정산 종료 |
