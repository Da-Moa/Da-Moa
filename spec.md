...
| FR-03 | 모든 활성 모임 참여자는 회차를 생성할 수 있다. 요청자를 `rounds.creator_id`에 저장하고 필수 회차 참여자로 포함해 활성 멤버 후보 중 최소 2명을 선택한다. 모임 생성자는 필수 회차 참여자가 아니며 미완료 회차 수를 하나로 제한하지 않는다. |
...
| FR-21 | 모임 상세의 본인 참여 회차는 최신순으로 처음 3개만 표시한다. 4개 이상이면 ‘더보기’를 눌러 전체 회차를 별도 모달에서 확인하며 홈·전체 정산 기록 목록에는 이 축약을 적용하지 않는다. 모달이 열린 동안 문서 스크롤을 잠그고 모달 끝의 스크롤이 뒤 페이지로 전달되지 않게 한다. |
| FR-22 | 정산 기록은 안내 문구 아래, 모임 상세의 본인 참여 회차는 제목 오른쪽 돋보기로 검색창을 열고 닫는다. 검색어는 공백 제거 후 1~100자이며 모임명 또는 회차명에 포함되는 본인 참여 회차만 대소문자 구분 없이 최신순으로 조회한다. 검색은 상태·모임·커서 조건과 함께 적용하고 검색창을 닫으면 검색어를 초기화한다. |
...
| `/home/groups/[groupId]` | 생성자 우선 현재 멤버 요약(최대 4명)과 5명 이상일 때의 전체 목록 모달, 모임명·회차명 검색이 가능한 본인 참여 회차 최신 3개와 4개 이상일 때의 전체 목록 모달. 모든 활성 참여자에게 본인을 필수로 포함하는 회차 생성 폼을 제공한다. 모임 생성자에게만 초대·모임 없애기를, 다른 참여자에게는 조건부 모임 나가기를 제공 |
...
- 모든 활성 모임 참여자는 자신을 포함한 최소 2명으로 회차를 만들 수 있다. 시작자를 빼거나 비활성·외부 사용자를 포함하면 거부하며 모임 생성자는 필수 참여자가 아니다. 응답의 `creatorId`·`isCreator`는 회차 생성자를, `groupCreatorId`는 모임 생성자를 정확히 구분한다.
...
- 정산 기록과 모임 상세에서 돋보기를 누르면 검색창이 나타난다. 회차명의 일부와 모임명의 일부가 각각 일치하는 회차만 조회되며 상태 필터·최신 3개 요약·전체 목록 모달·커서 페이지네이션도 검색 결과를 기준으로 동작한다. 검색창을 닫으면 전체 목록으로 돌아간다.
