...
try {
  ...
  await evaluate("document.querySelector('button[aria-label=\"내가 참여한 회차 검색\"]').click()")
  await waitFor("Boolean(document.querySelector('input[aria-label=\"내가 참여한 회차 검색어\"]'))")
  await fill('input[aria-label="내가 참여한 회차 검색어"]', '목록 검증 1')
  await waitFor("document.querySelectorAll('[aria-label=\"내가 참여한 회차 요약\"] > a.round-link').length === 1 && document.querySelector('[aria-label=\"내가 참여한 회차 요약\"]')?.textContent.includes('목록 검증 1')")
  assert.equal(await evaluate("Boolean(document.querySelector('button[aria-label=\"참여 회차 전체 보기\"]'))"), false)
  await fill('input[aria-label="내가 참여한 회차 검색어"]', groupName)
  await waitFor("document.querySelectorAll('[aria-label=\"내가 참여한 회차 요약\"] > a.round-link').length === 3 && Boolean(document.querySelector('button[aria-label=\"참여 회차 전체 보기\"]'))")
  await fill('input[aria-label="내가 참여한 회차 검색어"]', '')
  await waitFor("document.querySelectorAll('[aria-label=\"내가 참여한 회차 요약\"] > a.round-link').length === 3 && Boolean(document.querySelector('button[aria-label=\"참여 회차 전체 보기\"]'))")
  ...
  assert.equal(await evaluate("getComputedStyle(document.documentElement).overflow === 'hidden' && getComputedStyle(document.body).overflow === 'hidden' && getComputedStyle(document.querySelector('#group-rounds-dialog')).overscrollBehaviorY === 'contain'"), true)
  assert.equal(await evaluate("document.querySelectorAll('[aria-label=\"내가 참여한 회차 요약\"] > a.round-link').length"), 3)
  ...
  console.log('PASS group round list keeps the latest three and shows all rounds in a dialog')
  ...
  await waitFor("Boolean(document.querySelector('button[aria-label=\"정산 기록 검색\"]'))")
  assert.equal(await evaluate("(() => { const description = Array.from(document.querySelectorAll('p.help-text')).find(node => node.textContent.includes('참여했던 모든 회차')); const search = document.querySelector('button[aria-label=\"정산 기록 검색\"]'); return Boolean(description && search && (description.compareDocumentPosition(search) & Node.DOCUMENT_POSITION_FOLLOWING)); })()"), true)
  await evaluate("document.querySelector('button[aria-label=\"정산 기록 검색\"]').click()")
  await waitFor("Boolean(document.querySelector('input[aria-label=\"정산 기록 검색어\"]'))")
  assert.equal(await evaluate("(() => { const description = Array.from(document.querySelectorAll('p.help-text')).find(node => node.textContent.includes('참여했던 모든 회차')); const input = document.querySelector('input[aria-label=\"정산 기록 검색어\"]'); return Boolean(description && input && (description.compareDocumentPosition(input) & Node.DOCUMENT_POSITION_FOLLOWING)); })()"), true)
