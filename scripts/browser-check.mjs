...
try {
  ...
  await evaluate("window.__staleAlertSeen = false; window.__staleAlertObserver = new MutationObserver(() => { if (document.querySelector('.expense-form [role=alert]')) window.__staleAlertSeen = true }); window.__staleAlertObserver.observe(document.body, { childList: true, subtree: true })")
  await click('지출 저장')
  ...
  await setSession(payer.session)
  await navigate('/onboarding', '계좌 연결하기')
  assert.equal(await evaluate(hasText('이전 모임으로 자동 복귀하지 않으며 새 초대가 필요해요.')), true)
  assert.equal(await evaluate(hasText('먼저 금융결제원에서 계좌 연결에 동의해 주세요.')), true)
  assert.equal(await evaluate("Boolean(document.querySelector('[name=accountNumber], [name=birthDate], button[type=submit]'))"), false)
  ...
  console.log('PASS leaver history, soft withdrawal, OAuth required before rejoin, and no automatic membership restoration')
