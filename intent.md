...
- 금액은 직접 입력하고 영수증은 증거용 이미지만 업로드·저장한다. 영수증 없이 기록할 수도 있다.
...
- ‘전송’ 버튼은 회차를 잠그고 안내 화면으로 진행하는 동작이다. 실제 외부 메시지 전송·송금·입금 확인은 제공하지 않는다.
...
- 로그인한 사용자가 모임을 생성하거나 초대 링크로 참여한다. 생성자는 자동으로 모임 참여자에 포함한다.
- 초대 발송은 참여 완료가 아니다. 로그인 후 참여를 수락해야 하며, 초대 진입 중 로그인하더라도 해당 모임으로 복귀해야 한다.
- 일반 멤버 변경은 다음 회차부터 반영한다. 신규 멤버를 과거 지출에 자동 포함하지 않는다.
- 생성자가 안내 준비 단계에서 수행하는 사용자 제외는 이번 회차의 분배도 바꾸는 명시적인 예외다.
- 모임 생성자는 제외할 수 없고, 관리 권한 이전도 제공하지 않는다. 생성자 부재 시 다른 사람에게 관리 권한을 자동 승계하지 않는다.
- 일반 참여자는 자신이 제외되지 않은 미종료 회차에 참여 중이면 모임에서 나갈 수 없다. 그런 회차가 없으면 현재 모임 멤버십의 `group_members.left_at`을 설정해 다음 회차 후보에서 빠진다.
- 모임 생성자에게는 일반 나가기를 제공하지 않는다. 생성자의 본인 회차 참여 여부와 관계없이, 모임 전체에 `COMPLETED`가 아닌 회차가 하나라도 있으면 모임 없애기를 거부한다. 회차가 없거나 모든 회차가 `COMPLETED`이면 허용한다.
...
| 작업 | 권한·조건 |
|---|---|
| 기록 시작·정산 확정·전송·정산 종료 | 모임 생성자가 수행한다. |
| 지출 작성 | 해당 회차 참여자가 기록 단계에서 작성한다. |
| 지출 수정·삭제 | 기록 단계에서 **해당 지출 작성자 또는 모임 생성자**가 수행한다. 다른 일반 참여자는 수정·삭제할 수 없다. |
| 기록 단계로 다시 열기 | 모임 생성자만 가능하다. **미전송·미종료 회차**에 한한다. |
| 회차 전체 취소 | 모임 생성자만 가능하다. 미전송·미종료이며 현재 기록 단계인 회차에 한한다. |
| 사용자 제외 | 생성자가 전송 전에 수행하며, 결제자·부담자 유지와 특정 분배 관련 제한을 확인한다. |
| 모임 나가기 | 일반 참여자만 가능하다. 본인이 제외되지 않은 미종료 회차에 참여 중이면 거부하고, 그 외에는 현재 멤버십만 이탈 처리한다. |
| 모임 없애기 | 생성자만 가능하다. 모임 전체에 미종료 회차가 있으면 거부하고, 회차가 없거나 모두 완료됐으면 모든 활성 멤버십과 초대를 종료해 모임을 닫는다. 모임 행과 완료 회차는 보존하며 생성자에게 일반 나가기는 제공하지 않는다. |
| 완료 회차 변경 | **누구도 불가능하다.** 생성자도 재오픈·수정·삭제·취소할 수 없다. |
| 과거 정산 조회 | 인증한 사용자의 해당 회차 참여 이력을 확인한다. 모임 이탈 후에도 본인 참여 회차를 조회할 수 있다. |

작성자와 결제자는 다를 수 있다. A가 기록을 작성하고 B가 실제 결제했으며 B·C가 부담한다면, 기록 수정 권한은 A와 모임 생성자에게 있고 결제액은 B에게, 부담액은 B·C에게 반영한다. 생성자가 수정했다고 작성자·결제자를 생성자로 바꾸지 않는다.
...
- 날짜가 과거여도 **미종료·미전송**이면 생성자가 다시 열 수 있다. 정산 종료된 과거 회차는 영구적으로 읽기 전용이다.
...
flowchart TD
    ...
    C -->|생성자가 미전송·미종료 회차 다시 열기| B
    D -->|생성자가 수정 · 제외 처리로 복귀| B
    D -->|전송| E[회차 잠금 · 다시 열기 금지]
    E --> F[안내 화면 · 나머지 있으면 랜덤 돌리기]
    F --> G[사용자별 최종 안내 표시 · 링크 복사]
    G --> H[생성자가 정산 종료 · 읽기 전용]
    B -->|생성자가 회차 취소| X[회차 하드 삭제]
```
...
- 수정이 필요하면 생성자가 기록 단계로 다시 열고, 작성자 또는 생성자가 수정한 뒤 생성자가 재확정한다.
- 다시 여는 즉시 기존 결과는 안내 가능한 확정 상태에서 해제한다. 수정 중인 회차의 최종 안내 링크를 제공하지 않는다.
- 안내 확인만 취소했다면 확정 상태를 유지한다. 기록 단계로 돌아갔다면 재확정해야 한다.

### 잠금과 완료

- ‘전송’ 시 서버에서 상태를 확인하고 지출·분배 대상을 잠근다. 이후 생성자도 재오픈하거나 기록을 바꿀 수 없다.
- 잠금 후 미배분 나머지를 한 번 추첨해 최종 금액을 완성하는 것은 예정된 후속 단계다. 원본을 수정하거나 완료한 추첨을 다시 실행하는 것은 허용하지 않는다.
- 생성자가 별도로 **‘정산 종료’**를 표시한다. ‘전송’이나 링크 복사만으로 자동 종료하지 않는다.
- 정산 종료는 업무상 완료이며 실제 입금 검증을 의미하지 않는다. 해당 회차로 인한 회원탈퇴 제한은 종료 시 해제한다.
...
- 모임 생성자만 미전송·미종료 회차를 기록 단계에서 취소할 수 있다.
- 한 번 확정했다가 다시 연 회차도 현재 기록 단계라면 취소할 수 있다. ‘최초 확정 전’으로 제한하지 않는다.
- 확정 상태라면 먼저 생성자가 다시 열어야 한다. 전송했거나 정산 종료한 회차는 취소할 수 없다.
...
- 상품 정책상 금액 상한은 없다. 유효한 양의 금액과 통화별 소수 자릿수를 검증하고, 큰 금액도 입력·계산·저장·표시에서 정밀도를 보존한다.
...
생성자는 ‘전송’ 전 안내 준비 과정에서 제외 가능한 사용자를 제외할 수 있다. 이는 메시지 수신자만 빼는 동작이 아니라 **이번 회차의 부담 대상을 변경하는 동작**이다.

1. 생성자 권한, 대상자의 회차 참여 여부, 미전송·미종료 상태를 확인한다.
2. 대상자가 모임 생성자이면 제외를 거부한다.
...
6. 생성자가 기록 단계로 다시 연 뒤 **해당 작성자 또는 모임 생성자**가 관련 기록을 수정하고, 생성자가 제외를 다시 시도한다.
...
- 이탈자는 다음 회차의 기존 멤버 후보에서 제외한다. 이탈만으로 과거 지출·정산 결과·회차 참여 이력을 삭제하지 않는다.
...
- 모임 생성자는 자진 나가기를 사용할 수 없고 관리 권한도 이전하지 않는다. 생성자는 모임 없애기만 요청할 수 있다.
- 모임 없애기 차단 조건은 생성자의 회차 참여 여부가 아니라 모임 전체의 상태다. `status != COMPLETED`인 회차가 하나라도 있으면 거부하고, 회차가 없거나 모든 회차가 `COMPLETED`이면 허용한다.
...
- 이 계좌 공개 범위는 생성자에게도 동일하게 적용한다. 생성자라는 이유로 다른 사람의 모든 수취 계좌를 조회하지 않는다.
- 다른 참여자의 불필요한 계좌정보는 화면에서 숨기는 데 그치지 않고 서버 응답에서도 제외한다.
- 회차 지출 화면은 참여자 이름을 보여 주되, 송금 관계·금액은 접속한 사용자가 직접 보내거나 받는 관계만 그림으로 보여 준다. 모임 생성자도 같은 기준이며, 다른 참여자끼리의 관계는 서버 응답과 화면에서 제외한다. 지출이 추가·수정·삭제될 때마다 회차 전체를 다시 상계해 본인의 예상 관계를 갱신하고, 추첨 뒤에는 저장된 본인의 최종 관계로 교체한다. 계좌정보는 포함하지 않는다.
...
- 추후 계좌 실재 여부·예금주 비교 API를 추가한다. 현재 실제 송금·입금 확인과 외부 메시지 전송은 범위에 포함하지 않는다.
...
- 기존 모임에 다시 참여하려면 생성자가 제공한 유효한 초대 링크를 통해 새로 참여를 수락해야 한다. 과거 조회 링크나 과거 참여 이력만으로 모임 재참여를 승인하지 않는다.
- 다시 가입했다는 이유만으로 다음 회차의 기존 멤버 후보에 자동 포함하지 않는다. 초대 수락 후 현재 멤버로 참여할 수 있다.
- 모임 관리 작업은 과거 생성자 ID 일치만으로 허용하지 않고, 현재 활성 계정·멤버십과 생성자 권한을 확인한다.
...
- 지출 작성 중 다른 변경을 수신해도 작성 시작 시점의 `expectedVersion`을 유지해 자동으로 덮어쓰지 않는다.
...
| 모임·멤버십 | 생성자, 활성·이탈 멤버 관계, 초대 정보 |
| 회차 | 소속 모임, 이름, 생성 시 선택한 통화, 참여자, 기록·확정·잠금·종료 상태 |
| 지출 | 작성자, 결제자 한 명, 금액, 분배 방식, 부담자 목록, 증빙 이미지 |
| 정산·추첨 | 기본 부담액·나머지, 1회 추첨 결과, 최종 잔액·송금 목록 |
...
- 현재 ‘내가 받을 금액’은 1인 부담액을 표시한다. 개인 정산 잔액과 본인의 수취인 목록을 기준으로 표시해야 한다.
...
| 생성자 제외·결제자 겸 부담자 제외 | 제외를 거부하고 필요한 결제·부담 관계를 유지한다. |
| 특정 분배 관련 사용자 제외 | 제외를 막고 관련 내역을 팝업으로 안내한다. 내역을 선택하면 해당 기록을 펼쳐 하이라이트하며, 작성자 또는 생성자가 수정한 뒤 재시도한다. |
| 제외 후 2명 미만 | 제외를 거부한다. |
| 지출 0건 확정 | ‘지출 내역이 없습니다’를 표시하고 확정을 거부한다. |
| 다른 작성자의 기록 수정 | 기록 단계에서 작성자 또는 모임 생성자만 허용한다. 완료·전송 후에는 모두 거부한다. |
| 재오픈 회차 취소 | 생성자가 미전송·미종료 기록 단계에서 하드 삭제할 수 있다. |
...
| 생성자의 모임 나가기·없애기 | 일반 나가기는 거부한다. 생성자의 참여 여부와 관계없이 모임 전체에 미종료 회차가 있으면 없애기를 거부한다. 회차가 없거나 모두 완료됐으면 모임·회차는 보존하고 모든 활성 멤버십·초대를 종료해 모임을 닫는다. |
| 동일 카카오 식별자로 재가입 | 검증된 인증에 근거해 기존 회원을 복구한다. 과거 조회는 허용하고 모임 참여는 새 초대 수락을 요구한다. |
| 모든 개인 잔액이 0 | ‘송금할 금액 없음’을 표시한다. 정산 종료는 생성자가 별도로 처리한다. |

## 12. 결정 상태

기존에 남아 있던 회차 취소 권한·시점, 관리 권한 이전, 진행·완료 회차 수정 권한, 사용자별 계좌 공개·최신 조회, 단일 결제자 제한, 회원 소프트 삭제·동일 카카오 식별자 재가입, 일반 참여자의 조건부 모임 나가기와 생성자의 조건부 모임 닫기 정책은 모두 확정해 본문에 반영했다.
