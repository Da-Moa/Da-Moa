...
- 지원 통화는 **USD·KRW·JPY**다. 모임 생성 시 기준 통화를 선택하고 회차 생성 시 저장한다. 이미 생성한 회차의 통화는 변경할 수 없다.
...
KRW 모임에서 10,000원을 3명이 나누면 3,334원·3,333원·3,333원이며 추가 1원을 부담하는 사람이 랜덤이다. USD·JPY는 각 통화의 최소 단위로 같은 규칙을 적용한다.
...
| 모임·멤버십 | 생성자, 기준 통화, 활성·이탈 멤버 관계, 초대 정보 |
| 회차 | 소속 모임, 이름, 생성 시 통화, 참여자, 기록·확정·잠금·종료 상태 |
