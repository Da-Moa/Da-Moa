...
- 추첨 전에는 **기본 부담액 합계 + 미배분 나머지 = 지출 금액**이어야 한다. 이 값을 완성된 개인별 송금액으로 표시하지 않는다.
...
- 회차 지출 화면은 모든 참여자에게 참여자 이름과 최종 송금 관계·금액을 그림으로 보여 준다. 계좌정보는 포함하지 않으며, 최종 계산 전에는 금액 대신 계산 대기 상태를 표시한다.
...
| 특정 분배 관련 사용자 제외 | 제외를 막고 관련 내역을 하이라이트한다. 작성자 또는 생성자가 수정한 뒤 재시도한다. |
| 제외 후 2명 미만 | 제외를 거부한다. |
